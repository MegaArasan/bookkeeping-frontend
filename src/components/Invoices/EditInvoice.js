...
function Editinvoice({ invoiceData, setInvoiceData, customers, user, id }) {
  const [customer, setcustomer] = useState(invoiceData.customer);
  const [rates, setRates] = useState(invoiceData.rates);
  const [vat, setVat] = useState(invoiceData.vat);
  const [type, setType] = useState(invoiceData.type);
  ...
  const history = useHistory();
  const [selectedDate, setSelectedDate] = useState(invoiceData.selectedDate);
